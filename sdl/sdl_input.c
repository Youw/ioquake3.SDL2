...
static char KeyToShiftKey(char ch_in) {
	if (ch_in >= 'a' && ch_in <= 'z') {
		return ch_in - 'a' + 'A';
	} else
		switch (ch_in) {
	...
	keyNum_t *key, qboolean down )
{
	...
	*key = 0;
	...
	if (keymod & KMOD_SHIFT) {
		keysym->sym = KeyToShiftKey((char)keysym->sym);
	}
...
//			case SDLK_LMETA:        *key = K_COMMAND;       break;
			...
			case SDLK_LGUI:
			case SDLK_RGUI:         *key = K_SUPER;         break;
			...
			case SDLK_SYSREQ:       *key = K_SYSREQ;        break;
				...
				//now it st the same as SDLK_PAUSE
	...
	if (down && (nonASCII==qfalse))
	{
		unsigned char ch = (unsigned char)keysym->sym & 0xFF;
	...
	if( down && strlen( Key_KeynumToString( *key ) ) == 1 &&
		//i'm not shure that keysym->sym==0 is dead key, but it seems right
		keysym->sym == 0 )
	{
		if( in_keyboardDebug->integer )
...
static void IN_GobbleMotionEvents( void )
{
	...
	while( SDL_PeepEvents( dummy, 1, SDL_GETEVENT,
		SDL_MOUSEMOTION, SDL_MOUSEMOTION+3)) {
	}
}
...
static void IN_InitJoystick( void )
{
	...
	for (i = 0; i < total; i++)
	{
		Q_strcat(buf, sizeof(buf), SDL_JoystickName(SDL_JoystickOpen(i)));
	...
	Com_DPrintf("Name:       %s\n", SDL_JoystickName(SDL_JoystickOpen(in_joystickNo->integer)));
			...
			case SDL_MOUSEWHEEL:
				{
					...
					Com_QueueEvent(0, SE_KEY, b, qtrue, 0, NULL);
				}
				break;
...
//	keys[K_SCROLLOCK].down = 0;// keystate[SDL_SCANCODE_SCROLLLOCK];
//	keys[K_KP_NUMLOCK].down = 0;// keystate[SDL_SCANCODE_NUMLOCKCLEAR];
//	keys[K_CAPSLOCK].down = 0;// keystate[SDL_SCANCODE_CAPSLOCK];
}
